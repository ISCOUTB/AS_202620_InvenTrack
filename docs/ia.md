...
| 2026-09-06 | Reto Corte 1: Concurrencia y Medición de Inventario | Generación del script de pruebas asíncronas concurrentes con Pytest/HTTPX, apoyo en la redacción del ADR-0002 y estructuración del reporte de medición para el Reto de Corte 1. | Se rechazó la sugerencia de la IA de implementar Redis Distributed Locks o transacciones pesimistas SQL por considerar que agregaban infraestructura innecesaria al esqueleto actual en memoria. Se exigió una solución de exclusión mutua (Mutex/Lock por SKU) a nivel de aplicación para preservar la simplicidad del MVP y cumplir con $p95 \le 400\text{ ms}$. | Apoyo en la generación de pruebas de carga y redacción del ADR-0002. El equipo determinó el mecanismo de concurrencia, ejecutó las mediciones y validó la trazabilidad. |
