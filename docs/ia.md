...
| 2026-09-06 | Reto Corte 1: Concurrencia y Medición de Inventario | Generación del script de pruebas asíncronas concurrentes con Pytest/HTTPX, apoyo en la redacción del ADR-0002, estructuración del reporte de medición y documentación del diagnóstico técnico (síntoma, causa raíz y riesgo) y degradación controlada. | Se rechazó la sugerencia de la IA de implementar Redis Distributed Locks o transacciones pesimistas SQL por considerar que agregaban infraestructura innecesaria al esqueleto actual en memoria. Se exigió una solución de exclusión mutua (Mutex/Lock por SKU) a nivel de aplicación para preservar la simplicidad del MVP, respondiendo con HTTP 429/503 ante saturación y cumpliendo con $p95 \le 400\text{ ms}$. | Apoyo en la generación de pruebas de carga, estructuración del diagnóstico técnico y redacción del ADR-0002. El equipo determinó el mecanismo de concurrencia, ejecutó las mediciones y validó la trazabilidad. |
