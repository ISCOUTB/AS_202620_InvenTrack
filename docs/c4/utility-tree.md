...
Los atributos elegidos siguen el marco de clase "cinco atributos, cinco
...
| ESC-05 | Seguridad | Control de acceso por rol | (M, M) | La protección de datos personales es una restricción legal (C1), no solo una preferencia; el riesgo técnico es medio porque el mecanismo de autenticación aún no está decidido (depende del stack). |

Los escenarios con impacto de negocio alto (ESC-01, ESC-03) son los que
primero deben orientar decisiones arquitectónicas concretas (ADR). ESC-01
tiene además riesgo técnico alto, por lo que es el candidato natural para
la primera decisión documentada del proyecto.

**Nota sobre Usabilidad:** es relevante para el perfil de usuario descrito
en la ficha del problema, pero al aplicar este árbol de utilidad no
alcanzó la misma prioridad que Seguridad esta semana, ya que la protección
de datos personales es además una obligación legal. Puede convertirse en
un aspecto propio en semanas futuras si el equipo lo decide.
